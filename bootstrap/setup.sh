...
if [ -f "$BACKUP_KEY_FILE" ]; then
    ...
    if "$BACKUP_SCRIPT" "$BACKUP_KEY_FILE"; then
        ...
        # 失敗した場合 (exit 1) -> クラスタにも鍵がない
        echo "❌ Critical Error: Master key not found locally AND not found in the cluster."
        echo "   If this is a FRESH install (first time ever), please comment out the 'exit 1' below."
        echo "   If this is a recovery, you are missing the key file!"
